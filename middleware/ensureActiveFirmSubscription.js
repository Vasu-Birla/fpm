// middleware/ensureActiveFirmSubscription.js
import FirmSubscription from '../models/FirmSubscription.js';
import Plan from '../models/Plan.js';

export async function ensureActiveFirmSubscription(req, res, next) {
  try {
    ...
    const active = await FirmSubscription.findOne({
      ...
      include: [{
        ...
        as: 'plan',
        required: true,
        where: { is_active: true, solution_key: 'core' }, // 👈 same hardening
        attributes: ['plan_id']
      }]
    });
    ...
    console.log('ensureActiveFirmSubscription error:', e);
